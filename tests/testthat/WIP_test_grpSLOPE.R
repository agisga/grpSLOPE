...
X <- as.matrix(read.table("./tests/testthat/test_data/gaussianMC_test_mat.txt"))
X <- matrix(rnorm(15000,sd=sqrt(5000)), 5000, 30)
b <- c(0, 1.4, 0, 0, 0, 0, 0, 1.2, 1.5, 1.1, 0, 1.2, 0, 1.1, 1.2, 1.1,
       1.2, 1.1, 1.5, 0, 1.5, 1.1, 0, 0, 1.4, 1.5, 0, 0, 0, 0)
y <- X %*% b + rnorm(nrow(X))
...
sol <- grpSLOPE(X=X, y=y, group=group, fdr=0.1, lambda="gaussianMC", sigma=1, verbose=TRUE)
solChi <- grpSLOPE(X=X, y=y, group=group, fdr=0.1, lambda="chiMean", sigma=1, verbose=TRUE)
