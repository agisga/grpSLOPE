...
As an introduction to the R package, in the following we will walk through a usage example. First, we will simulate some data, before we feed it into `grpSLOPE`, and examine the output.
...
So, the truly significant groups are:

```{r}
group.names[ind.relevant]
```
...
for (j in ind.relevant) {
  # for the significant groups, generate effect sizes from the Uniform(0,1) distribution
...
model <- grpSLOPE::grpSLOPE(X=X, y=y, group=group, fdr=0.1)
```
...
The $\lambda$ sequences "mean" and "max" can be used together with the options `orthogonalize = FALSE` and `normalize = FALSE`, when the columns of the model matrix are exactly orthogonal to each other ("max" is more conservative than "mean").
...
* [Brzyski, D., Su, W., & Bogdan, M. (2015). *Group SLOPE &mdash; adaptive selection of groups of predictors.* arXiv preprint arXiv:1511.09078.](http://arxiv.org/abs/1511.09078)
